...
function __getRLQuery($id, $module, $relatedModule, $queryParameters, $user) {
	...
	case 'ModComments':
		...
		switch ($module) {
		case 'HelpDesk';
			$query = "SELECT vtiger_crmentity.createdtime, vtiger_modcomments.commentcontent, vtiger_users.user_name
						FROM vtiger_modcomments
						INNER JOIN vtiger_crmentity ON vtiger_modcomments.modcommentsid = vtiger_crmentity.crmid
						LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id
						LEFT JOIN vtiger_groups ON vtiger_crmentity.smownerid = vtiger_groups.groupid
						WHERE vtiger_crmentity.deleted = 0 AND vtiger_modcomments.modcommentsid > 0 AND vtiger_modcomments.related_to = $crmid
		...
		default:
			...
			case 'All':$queryCriteria = '';
				break;
			case 'Last5':$queryCriteria = sprintf(" ORDER BY %s.%s DESC LIMIT 5", $entityInstance->table_name, $entityInstance->table_index);
				break;
			case 'Mine':$queryCriteria = ' AND vtiger_crmentity.smcreatorid=' . $current_user->id;
				break;
			}
