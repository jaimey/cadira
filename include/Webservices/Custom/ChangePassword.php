...
function vtws_changePassword($id, $oldPassword, $newPassword, $confirmPassword, $user)
{
	...
	if ($idComponents[1] == $user->id || is_admin($user)) {
		...
		if (! is_admin($user)) {
			if (empty($oldPassword)) {
				throw new WebServiceException(
					...
					vtws_getWebserviceTranslatedString('LBL_'.WebServiceErrorCode::$INVALIDOLDPASSWORD)
				);
			}
		...
		if (isPasswordStrong($newPassword)) {
			if (strcmp($newPassword, $confirmPassword) === 0) {
				...
				if (! $success) {
					throw new WebServiceException(
						...
						vtws_getWebserviceTranslatedString('LBL_'.WebServiceErrorCode::$CHANGEPASSWORDFAILURE)
					);
				}
			} else {
				throw new WebServiceException(
			...
			throw new WebServiceException(
				WebServiceErrorCode::$CHANGEPASSWORDFAILURE,
				vtws_getWebserviceTranslatedString('LBL_'.WebServiceErrorCode::$PASSWORDNOTSTRONG)
			);
		}
