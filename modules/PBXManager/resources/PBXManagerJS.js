 ...
 * All Rights Reserved. - Jaime File, create buttons at pop-up by grousso
	...
	showPBXIncomingCallPopup: function (record) {
		...
		var params = {
			...
			'title': '<div class="row-fluid pbxcall" id="pbxcall_' + record.pbxmanagerid + '" callid=' + record.pbxmanagerid + ' style="color:black">' +
				//εμφανίζει το κουμπι κλεισίματος
				'<p><div><button class="removeDiv">&#10006;</button></div></p>' +
	...
	checkIfRelatedModuleRecordExist: function (record) {
		switch (record.callername) {
			case null:
				...
				break;
			default:
				jQuery('#caller', '#pbxcall_' + record.pbxmanagerid + '').html(app.vtranslate('JS_PBX_CALL_FROM') + ' :&nbsp;<a href="index.php?module=' + record.customertype + '&view=Detail&record=' + record.customer + '">' + record.callername + '</a><p></p><div class="btn-group btn-group-toggle"><a class="btn btn-primary" href="index.php?module=Project&view=Edit&app=MARKETING&sourceModule=Contacts&sourceRecord=' + record.customer + '&relationOperation=true&linktoaccountscontacts=' + record.customer + '&returnmode=showRelatedList&returntab_label=Projects&returnrecord=' + record.customer + '&returnmodule=Contacts" role="button">Add Project</a><a class="btn btn-primary" href="index.php?module=HelpDesk&view=Edit&sourceModule=Contacts&sourceRecord=' + record.customer + '&relationOperation=true&contact_id=' + record.customer + '&returnmode=showRelatedList&returntab_label=HelpDesk&returnrecord=' + record.customer + '" role="button">Add Ticket</a><a class="btn btn-primary" href="index.php?module=Calendar&view=Edit&mode=Events&calendarModule=Events&defaultCallDuration=5&defaultOtherEventDuration=30&eventstatus=Planned&activitytype=Meeting&sourceModule=Contacts&sourceRecord=' + record.customer + '&relationOperation=true&contact_id=' + record.customer + '&returnmode=showRelatedList&returntab_label=Activities&returnrecord=' + record.customer + '&returnmodule=Contacts&returnview=Detail&returnrelatedModuleName=Calendar&app=MARKETING" role="button">Add Event</a><a class="btn btn-primary" href="index.php?module=Calendar&view=Edit&mode=Calendar&calendarModule=Calendar&defaultCallDuration=5&defaultOtherEventDuration=30&sourceModule=Contacts&sourceRecord=' + record.customer + '&relationOperation=true&contact_id=' + record.customer + '&returnmode=showRelatedList&returntab_label=Activities&returnrecord=' + record.customer + '&returnmodule=Contacts&returnview=Detail&returnrelatedModuleName=Calendar&app=MARKETING" role="button">Add Task</a></div>');
